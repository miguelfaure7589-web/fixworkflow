...
const STATUS_BG: Record<string, string> = { pending: "#fef3c7", contacted: "rgba(67,97,238,0.07)", converted: "rgba(16,185,129,0.07)", declined: "var(--bg-subtle)" };
const STATUS_FG: Record<string, string> = { pending: "#92400e", contacted: "#4361ee", converted: "#10b981", declined: "var(--text-muted)" };
const STATUS_BD: Record<string, string> = { pending: "#fcd34d", contacted: "rgba(67,97,238,0.15)", converted: "rgba(16,185,129,0.15)", declined: "var(--border-default)" };
...
const FUNNEL_BG: Record<string, string> = { signed_up: "var(--bg-subtle)", diagnosis_done: "rgba(245,158,11,0.1)", onboarded: "rgba(67,97,238,0.1)", active: "rgba(16,185,129,0.1)", pro: "#6366f1" };
const FUNNEL_FG: Record<string, string> = { signed_up: "var(--text-muted)", diagnosis_done: "#d97706", onboarded: "#4361ee", active: "#10b981", pro: "#fff" };
...
const card: React.CSSProperties = { background: "var(--bg-card)", borderRadius: 14, border: "1px solid var(--border-default)", padding: 20 };
const thStyle: React.CSSProperties = { padding: "12px 16px", textAlign: "left" as const, fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" as const, letterSpacing: 0.5 };
const tdStyle: React.CSSProperties = { padding: "14px 16px", fontSize: 13, color: "var(--text-secondary)" };
      ...
      {entries.map(([pl, cnt]) => (
        <div key={pl}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", textTransform: "capitalize" as const }}>{pl.replace(/_/g, " ")}</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)" }}>{cnt} clicks</span>
          </div>
          <div style={{ height: 8, background: "var(--bg-subtle)", borderRadius: 4, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 4, width: (cnt / max * 100) + "%", background: PL_COLOR[pl] || "var(--text-muted)" }} />
          </div>
        </div>
      ))}
    </div>
...
function MetricChartModal({ metricKey, label, color, currentValue, history, onClose }: {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 28, maxWidth: 640, width: "100%", margin: "0 16px", boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>{label}</h2>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6 }}>
              <span style={{ fontSize: 28, fontWeight: 900, color: "var(--text-primary)" }}>{currentValue}</span>
              {changeStr && (
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          {([7, 30, 90] as const).map(d => (
            <button key={d} onClick={() => setRange(d)} style={{
              padding: "6px 14px", borderRadius: 7, border: "1px solid " + (range === d ? color : "var(--border-default)"),
              background: range === d ? color + "14" : "var(--bg-card)", color: range === d ? color : "var(--text-muted)",
          ...
          {filtered.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={filtered}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: "var(--text-muted)" }} tickFormatter={v => { const d = new Date(v); return (d.getMonth() + 1) + "/" + d.getDate(); }} />
...
function EditUserModal({ user, onClose, onSave }: { user: AdminUser; onClose: () => void; onSave: (data: Record<string, unknown>) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 28, maxWidth: 480, width: "100%", margin: "0 16px", boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>Edit User</h2>
        ...
        {[
          ...
          <div key={f.label} style={{ marginBottom: 14 }}>
            ...
            <input value={f.value} onChange={e => f.set(e.target.value)} type={f.type || "text"}
              style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border-default)", fontSize: 13, color: "var(--text-primary)", boxSizing: "border-box" }} />
          </div>
        ))}
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 4 }}>Stage</label>
          <select value={stage} onChange={e => setStage(e.target.value)}
            style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border-default)", fontSize: 13, color: "var(--text-primary)", background: "var(--bg-card)", cursor: "pointer" }}>
            {Object.entries(FUNNEL_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
        </div>
        <button onClick={handleSave} disabled={saving}
          style={{ width: "100%", padding: "10px 20px", borderRadius: 10, border: "none", background: saving ? "var(--border-default)" : "#4361ee", color: saving ? "var(--text-muted)" : "#fff", fontSize: 14, fontWeight: 700, cursor: saving ? "default" : "pointer" }}>
...
function DeleteConfirmModal({ userName, onClose, onConfirm }: { userName: string; onClose: () => void; onConfirm: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "var(--bg-card)", borderRadius: 16, padding: 28, maxWidth: 420, width: "100%", margin: "0 16px", boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }} onClick={e => e.stopPropagation()}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 12px" }}>Delete User</h2>
        <p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.6, margin: "0 0 20px" }}>
          Are you sure you want to delete <strong>{userName}</strong>? This action cannot be undone.
        </p>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: "10px 20px", borderRadius: 10, border: "1px solid var(--border-default)", background: "var(--bg-card)", color: "var(--text-secondary)", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
          <button onClick={() => { setDeleting(true); onConfirm(); }} disabled={deleting}
            style={{ flex: 1, padding: "10px 20px", borderRadius: 10, border: "none", background: deleting ? "var(--border-default)" : "#ef4444", color: deleting ? "var(--text-muted)" : "#fff", fontSize: 14, fontWeight: 700, cursor: deleting ? "default" : "pointer" }}>
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-subtle)" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "12px 16px" : "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Link href="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
              ...
              {!isMobile && <span style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)" }}>FixWorkFlow</span>}
            </Link>
          ...
          {!isMobile && (
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <Link href="/settings" style={{ fontSize: 13, color: "var(--text-secondary)", textDecoration: "none", fontWeight: 500 }}>Settings</Link>
      ...
      {isMobile && menuOpen && (
        <div style={{ position: "fixed", top: 56, left: 0, right: 0, bottom: 0, zIndex: 40, background: "var(--bg-card)", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 4, overflowY: "auto" }}>
          ...
          <Link href="/settings" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "14px 12px", fontSize: 16, color: "var(--text-primary)", textDecoration: "none", borderRadius: 10, fontWeight: 500 }}>Settings</Link>
          <Link href="/admin/referrals" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "14px 12px", fontSize: 16, color: "var(--text-primary)", textDecoration: "none", borderRadius: 10, fontWeight: 500 }}>Credit Referrals</Link>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "16px" : 24 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: isMobile ? 16 : 24, gap: 12, flexWrap: "wrap" }}>
          <div>
            <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>Admin Dashboard</h1>
            <p style={{ fontSize: isMobile ? 12 : 13, color: "var(--text-muted)", marginTop: 4 }}>FixWorkFlow business metrics</p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleRefresh} disabled={refreshing} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "1px solid var(--border-default)", background: "var(--bg-card)", fontSize: 13, fontWeight: 600, color: "var(--text-secondary)", cursor: "pointer", opacity: refreshing ? 0.6 : 1 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: isMobile ? 12 : 16, marginBottom: isMobile ? 16 : 24 }}>
            ...
            <div style={card}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>Funnel Drop-off</div>
        ...
        {integrationHealth && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Integration Health</span>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: isMobile ? 12 : 16, marginBottom: 16 }}>
              <div style={card}>
                <div style={{ fontSize: 32, fontWeight: 900, color: "var(--text-primary)", lineHeight: 1 }}>{integrationHealth.totalConnected}</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)", marginTop: 4 }}>Connected Integrations</div>
              ...
              <div style={card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>Last Sync</div>
                {integrationHealth.lastSync ? (
                  ...
                  <div style={{ fontSize: 13, color: "var(--text-muted)" }}>No syncs yet</div>
                )}
              </div>
              <div style={card}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)", marginTop: 4 }}>Failed Syncs (7d)</div>
                  </div>
                  <button
                    onClick={async () => {
                      setSyncRunning(true);
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "1px solid #4361ee", background: syncRunning ? "var(--bg-subtle)" : "transparent", fontSize: 12, fontWeight: 600, color: "#4361ee", cursor: syncRunning ? "not-allowed" : "pointer", opacity: syncRunning ? 0.6 : 1 }}
                  >
            ...
            {integrationHealth.failedSyncs.length > 0 && (
              <div style={{ ...card, padding: 0, overflow: "hidden" }}>
                <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border-default)", fontSize: 13, fontWeight: 700, color: "#ef4444" }}>Failed Syncs</div>
                  ...
                  <tbody>
                    {integrationHealth.failedSyncs.map(f => (
                      <tr key={f.id} style={{ borderBottom: "1px solid var(--border-light)" }}>
                        ...
                        <td style={{ ...tdStyle, fontWeight: 600, color: "var(--text-primary)", textTransform: "capitalize" as const }}>{f.provider === "stripe-data" ? "Stripe" : f.provider}</td>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Credit Referrals</span>
            ...
            <select value={refFilter} onChange={e => setRefFilter(e.target.value)} style={{ marginLeft: "auto", padding: "6px 10px", borderRadius: 7, border: "1px solid var(--border-default)", fontSize: 12, color: "var(--text-secondary)", cursor: "pointer", background: "var(--bg-card)" }}>
              ...
              <tbody>
                {referrals.map(r => (
                  <tr key={r.id} style={{ borderBottom: "1px solid var(--border-light)" }}>
                    ...
                    <td style={{ ...tdStyle, color: "var(--text-muted)" }}>{r.bestTimeToCall || "\u2014"}</td>
                    <td style={{ ...tdStyle, color: "var(--text-muted)", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.notes || ""}>{r.notes ? (r.notes.length > 60 ? r.notes.slice(0, 60) + "\u2026" : r.notes) : "\u2014"}</td>
                    <td style={{ ...tdStyle, fontWeight: 600, color: r.userScore ? "var(--text-primary)" : "var(--text-muted)" }}>{r.userScore ?? "\u2014"}</td>
                    <td style={tdStyle}>
                      <select value={r.status} onChange={e => handleStatusChange(r.id, e.target.value)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid " + (STATUS_BD[r.status] || "var(--border-default)"), fontSize: 12, fontWeight: 600, cursor: "pointer", background: STATUS_BG[r.status] || "var(--bg-subtle)", color: STATUS_FG[r.status] || "var(--text-muted)" }}>
                        {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </td>
                  </tr>
                ))}
                {referrals.length === 0 && <tr><td colSpan={8} style={{ padding: "32px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No referrals yet.</td></tr>}
              </tbody>
            </table>
            </div>
          </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Affiliate Performance</span>
            <select value={affRange} onChange={e => setAffRange(e.target.value)} style={{ marginLeft: "auto", padding: "6px 10px", borderRadius: 7, border: "1px solid var(--border-default)", fontSize: 12, color: "var(--text-secondary)", cursor: "pointer", background: "var(--bg-card)" }}>
          ...
          {affData && (affData.topProducts.length > 0 || affData.recentClicks.length > 0) ? (
            <>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 12 : 16, marginBottom: 16 }}>
                <div style={{ ...card, padding: 0, overflow: "hidden" }}>
                  <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-default)", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>Top Products</div>
                  <div style={{ overflowX: "auto" }}>
                    ...
                    <tbody>
                      {affData.topProducts.map((p, i) => (
                        <tr key={p.name} style={{ borderBottom: "1px solid var(--border-light)" }}>
                          <td style={{ ...tdStyle, fontWeight: 600, color: "var(--text-muted)" }}>{i + 1}</td>
                          <td style={{ ...tdStyle, fontWeight: 600, color: "var(--text-primary)" }}>{p.name}</td>
                          <td style={{ ...tdStyle, textAlign: "right" as const, fontWeight: 700 }}>{p.totalClicks}</td>
                          <td style={tdStyle}>
                            {Object.entries(p.placements).map(([pl, cnt]) => (
                              <span key={pl} style={{ fontSize: 10, fontWeight: 600, padding: "1px 5px", borderRadius: 3, marginRight: 4, background: (PL_COLOR[pl] || "var(--text-muted)") + "14", color: PL_COLOR[pl] || "var(--text-muted)" }}>
                                {pl.replace(/_/g, " ")} ({cnt})
                              </span>
                            ))}
                          </td>
                ...
                <div style={card}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 16 }}>Clicks by Placement</div>
                  {Object.keys(affData.byPlacement).length > 0 ? <PlacementBars data={affData.byPlacement} /> : <div style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", padding: 20 }}>No placement data yet.</div>}
                </div>
              </div>
              {affData.recentClicks.length > 0 && (
                <div style={{ ...card, padding: 0, overflow: "hidden" }}>
                  <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border-default)", fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>Recent Clicks</div>
                  <div style={{ maxHeight: 320, overflowY: "auto" as const }}>
                    {affData.recentClicks.map((c, i) => (
                      <div key={i} style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: isMobile ? 6 : 12, padding: isMobile ? "8px 12px" : "10px 16px", borderBottom: "1px solid var(--border-light)", fontSize: 12 }}>
                        <span style={{ color: "var(--text-muted)", minWidth: isMobile ? 60 : 80 }}>{relTime(c.timestamp)}</span>
                        <span style={{ color: "var(--text-secondary)" }}>{c.userEmail}</span>
                        <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{c.productName}</span>
                        <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 600, padding: "2px 6px", borderRadius: 4, background: (PL_COLOR[c.placement] || "var(--text-muted)") + "14", color: PL_COLOR[c.placement] || "var(--text-muted)" }}>{c.placement.replace(/_/g, " ")}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div style={{ ...card, textAlign: "center", padding: "32px 20px" }}>
              <div style={{ fontSize: 13, color: "var(--text-muted)" }}>Affiliate click tracking will appear here once users start clicking recommendations.</div>
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Feedback</span>
                ...
                <tbody>
                  {feedbackItems.map(f => (
                    <tr key={f.id} style={{ borderBottom: "1px solid var(--border-light)" }}>
                      ...
                      <td style={tdStyle}><span style={{ fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 5, background: f.type === "bug" ? "rgba(239,68,68,0.08)" : f.type === "feature" ? "rgba(67,97,238,0.08)" : "var(--bg-subtle)", color: f.type === "bug" ? "#ef4444" : f.type === "feature" ? "#4361ee" : "var(--text-muted)" }}>{f.type}</span></td>
                  ...
                  {feedbackItems.length === 0 && <tr><td colSpan={5} style={{ padding: "32px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No feedback yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Reviews</span>
          ...
          <div style={{ ...card, padding: 0, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                ...
                <tbody>
                  {reviewItems.map(rv => (
                    <tr key={rv.id} style={{ borderBottom: "1px solid var(--border-light)" }}>
                      <td style={{ ...tdStyle, whiteSpace: "nowrap", fontSize: 12 }}>{fmtDate(rv.createdAt)}</td>
                      <td style={tdStyle}>
                        <div>
                          <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{rv.userName || "\u2014"}</span>
                          {rv.userEmail && <span style={{ fontSize: 11, color: "var(--text-muted)", marginLeft: 6 }}>{rv.userEmail}</span>}
                        </div>
                      </td>
                      <td style={tdStyle}>
                        <div style={{ display: "flex", gap: 2 }}>
                          {[1, 2, 3, 4, 5].map(s => (
                            <Star key={s} size={14} fill={s <= rv.rating ? "#facc15" : "none"} color={s <= rv.rating ? "#facc15" : "var(--border-default)"} strokeWidth={1.5} />
                          ))}
                        </div>
                      </td>
                      <td style={{ ...tdStyle, maxWidth: 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={rv.comment || ""}>
                        {rv.comment || <span style={{ color: "var(--text-muted)" }}>No comment</span>}
                      </td>
                    </tr>
                  ))}
                  {reviewItems.length === 0 && <tr><td colSpan={4} style={{ padding: "32px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No reviews yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, flexWrap: "wrap" }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)" }}>Users</span>
            ...
            <div style={{ marginLeft: "auto", position: "relative" }}>
              ...
              <input
                ...
                style={{ padding: "8px 12px 8px 32px", borderRadius: 8, border: "1px solid var(--border-default)", fontSize: 13, color: "var(--text-primary)", width: isMobile ? 200 : 260, background: "var(--bg-card)" }}
              />
            </div>
          </div>
          <div style={{ ...card, padding: 0, overflow: "hidden" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800 }}>
              ...
              <tbody>
                {filteredUsers.map(u => {
                  ...
                  const fbg = FUNNEL_BG[u.funnelStage] || "var(--bg-subtle)";
                  const ffg = FUNNEL_FG[u.funnelStage] || "var(--text-muted)";
                  return (
                    <tr key={u.id} style={{ borderBottom: "1px solid var(--border-light)" }}>
                      ...
                      <td style={{ ...tdStyle, fontWeight: 600, color: u.score ? "var(--text-primary)" : "var(--text-muted)" }}>{u.score ?? "\u2014"}</td>
                      ...
                      <td style={{ ...tdStyle, textTransform: "capitalize" as const, color: u.usesPersonalCredit ? "var(--text-primary)" : "var(--text-muted)" }}>{u.usesPersonalCredit || "\u2014"}</td>
                      <td style={tdStyle}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => setEditingUser(u)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border-default)", background: "var(--bg-card)", cursor: "pointer", color: "#4361ee" }} title="Edit user">
                            <Pencil size={14} />
                          </button>
                          <button onClick={() => setDeletingUser(u)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border-default)", background: "var(--bg-card)", cursor: "pointer", color: "#ef4444" }} title="Delete user">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
                ...
                {filteredUsers.length === 0 && <tr><td colSpan={7} style={{ padding: "32px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>{userSearch ? "No users match your search." : "No users yet."}</td></tr>}
              </tbody>
            </table>
            </div>
          </div>
...
function FunnelRow({ label, val, total }: { label: string; val: number; total: number }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
      ...
      <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{val} <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>({pct(val, total)})</span></span>
    </div>
  );
}
