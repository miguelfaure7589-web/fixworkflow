...
import ScorePreviewCard, { ScoreRing, PillarBars } from './ScorePreviewCard';
...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: 'var(--font-outfit, var(--font-geist-sans)), sans-serif', color: 'var(--text-primary)' }}>
      {/* NAV */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, height: 60, padding: `0 ${px}px`,
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        background: scrollY > 20 ? 'var(--nav-bg)' : 'var(--nav-bg-subtle)',
        ...
        {!isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            <Link href="/pricing" style={{ fontSize: 14, color: 'var(--text-secondary)', textDecoration: 'none' }}>Pricing</Link>
            <Link href="/blog" style={{ fontSize: 14, color: 'var(--text-secondary)', textDecoration: 'none' }}>Blog</Link>
            <a href="#reviews" style={{ fontSize: 14, color: 'var(--text-secondary)', textDecoration: 'none' }}>Reviews</a>
            {isLoggedIn ? (
              <>
        ...
        {/* Mobile hamburger */}
        {isMobile && (
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', padding: 8, cursor: 'pointer', color: 'var(--text-primary)' }}>
      ...
      {/* Mobile menu overlay */}
      {isMobile && menuOpen && (
        <div style={{ position: 'fixed', top: 60, left: 0, right: 0, bottom: 0, zIndex: 99, background: 'var(--bg-card)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
          <Link href="/pricing" onClick={() => setMenuOpen(false)} style={{ display: 'block', padding: '14px 12px', fontSize: 16, color: 'var(--text-primary)', textDecoration: 'none', borderRadius: 10, fontWeight: 500 }}>Pricing</Link>
          <Link href="/blog" onClick={() => setMenuOpen(false)} style={{ display: 'block', padding: '14px 12px', fontSize: 16, color: 'var(--text-primary)', textDecoration: 'none', borderRadius: 10, fontWeight: 500 }}>Blog</Link>
          <a href="#reviews" onClick={() => setMenuOpen(false)} style={{ display: 'block', padding: '14px 12px', fontSize: 16, color: 'var(--text-primary)', textDecoration: 'none', borderRadius: 10, fontWeight: 500 }}>Reviews</a>
          {isLoggedIn ? (
            <>
            ...
            <>
              <Link href="/login" onClick={() => setMenuOpen(false)} style={{ display: 'block', padding: '14px 12px', fontSize: 16, color: 'var(--text-primary)', textDecoration: 'none', borderRadius: 10, fontWeight: 500 }}>Sign In</Link>
      ...
      {/* HERO */}
      <section style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border-default)', paddingTop: isMobile ? 80 : 120, paddingBottom: isMobile ? 48 : 80, minHeight: isMobile ? 'auto' : '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ maxWidth: 1100, width: '100%', margin: '0 auto', padding: `0 ${px}px`, display: 'flex', flexDirection: isSmall ? 'column' : 'row', gap: isMobile ? 32 : 70, alignItems: 'center' }}>
          <div style={{ flex: 1, opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.8s, transform 0.8s', textAlign: isMobile ? 'center' : 'left' }}>
            ...
            <p style={{ fontSize: isMobile ? 15 : 17, color: 'var(--text-secondary)', maxWidth: isMobile ? undefined : 440, margin: isMobile ? '0 auto 28px' : '0 0 28px', lineHeight: 1.6 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 12 }}>Takes 3 minutes · Personalized playbook · No credit card</p>
            <div style={{ paddingTop: 24, borderTop: '1px solid var(--border-light)', marginTop: 32, display: 'flex', alignItems: 'center', gap: 12, justifyContent: isMobile ? 'center' : 'flex-start', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex' }}>
                {avatars.map((a, i) => (
                  <div key={i} style={{ width: 28, height: 28, borderRadius: '50%', background: a.color, border: '2px solid var(--bg-card)', marginLeft: i > 0 ? -8 : 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>{a.initials}</div>
                ))}
              </div>
              <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Trusted by 850+ businesses</span>
            </div>
          </div>
          {!isMobile && (
      ...
      {/* PAIN POINTS */}
      <section style={{ background: 'var(--bg-elevated)', padding: `${isMobile ? 60 : 100}px ${px}px` }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-muted)', maxWidth: 500, margin: '0 auto 40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 14 }}>
            {painPoints.map((p, i) => (
              <div key={i} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
                style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-default)', padding: isMobile ? '20px 18px' : '24px 22px', textAlign: 'left', cursor: 'default',
          ...
          <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginTop: 36 }}>
      ...
      {/* PRODUCT PREVIEW */}
      <section style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border-light)', borderBottom: '1px solid var(--border-light)', padding: `${isMobile ? 48 : 80}px ${px}px` }}>
        <div style={{ maxWidth: 960, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontSize: isMobile ? 24 : 34, fontWeight: 900, margin: '0 0 10px' }}>This is what you&apos;ll see in 3 minutes.</h2>
          <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-secondary)', margin: '0 0 36px' }}>Not a vague report. A specific score, your weakest pillar, and a playbook to fix it.</p>
          <div style={{ background: 'var(--bg-elevated)', borderRadius: isMobile ? 14 : 18, border: '1px solid var(--border-default)', boxShadow: '0 8px 32px rgba(0,0,0,0.04)', padding: isMobile ? '20px 16px' : '28px 32px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 20 : 28, textAlign: 'left' }}>
            ...
            <div style={{ flex: 1, borderLeft: isMobile ? 'none' : '1px solid var(--border-default)', borderTop: isMobile ? '1px solid var(--border-default)' : 'none', paddingLeft: isMobile ? 0 : 28, paddingTop: isMobile ? 20 : 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.4, color: 'var(--text-muted)', textTransform: 'uppercase' as const }}>Your Playbook</span>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>5 steps · 10–25% revenue increase in 30 days</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {playbookSteps.map((s, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <div style={{ width: 20, height: 20, borderRadius: 6, background: s.done ? '#10b981' : 'var(--border-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {s.done && <Check style={{ width: 12, height: 12, color: '#fff', strokeWidth: 3 }} />}
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: s.done ? 'var(--text-muted)' : 'var(--text-primary)', textDecoration: s.done ? 'line-through' : 'none' }}>{s.label}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>{s.time}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section style={{ background: 'var(--bg-elevated)', padding: `${isMobile ? 60 : 100}px ${px}px` }}>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-muted)', margin: '0 0 40px' }}>No consultants. No guesswork. Just your numbers and a plan.</p>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 16 }}>
            {howSteps.map((s) => (
              <div key={s.num} style={{ flex: 1, position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-default)', padding: isMobile ? '22px 20px' : '28px 24px', textAlign: 'left' }}>
      ...
      {/* SOCIAL PROOF */}
      <section id="reviews" style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border-light)', padding: `${isMobile ? 48 : 80}px ${px}px` }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
          {realStats && realStats.totalUsers >= 10 && (
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : `repeat(${realStats.totalReviews > 0 ? 3 : 2}, 1fr)`, gap: isMobile ? 20 : 52, marginBottom: 48 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Businesses scored</div>
              </div>
              {realStats.totalReviews > 0 && (
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Average rating ({realStats.totalReviews} reviews)</div>
                </div>
              )}
              <div>
                <div style={{ fontSize: isMobile ? 22 : 26, fontWeight: 900, letterSpacing: -0.5 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Scores generated</div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 14 }}>
            {reviewData.map((r) => (
              <div key={r.initials} style={{ flex: 1, background: 'var(--bg-elevated)', borderRadius: 14, border: '1px solid var(--border-default)', padding: isMobile ? 18 : 22, textAlign: 'left' }}>
                ...
                <div style={{ borderTop: '1px solid var(--border-default)', paddingTop: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      {/* ABOUT */}
      <section style={{ background: 'var(--bg-elevated)', borderTop: '1px solid var(--border-default)', padding: `${isMobile ? 60 : 80}px ${px}px` }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ fontSize: isMobile ? 24 : 32, fontWeight: 900, letterSpacing: -0.8, margin: '0 0 12px', textAlign: 'center' }}>
            About FixWorkFlow
          </h2>
          <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-muted)', textAlign: 'center', margin: '0 0 40px', maxWidth: 600, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ borderLeft: '4px solid #4361ee', paddingLeft: isMobile ? 16 : 24, paddingTop: 2, paddingBottom: 2 }}>
              <div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>What is FixWorkFlow?</div>
              <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-secondary)', lineHeight: 1.7, margin: 0 }}>
            ...
            <div style={{ borderLeft: '4px solid #4361ee', paddingLeft: isMobile ? 16 : 24, paddingTop: 2, paddingBottom: 2 }}>
              <div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Who is it for?</div>
              <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-secondary)', lineHeight: 1.7, margin: 0 }}>
            ...
            <div style={{ borderLeft: '4px solid #4361ee', paddingLeft: isMobile ? 16 : 24, paddingTop: 2, paddingBottom: 2 }}>
              <div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Why it works</div>
              <p style={{ fontSize: isMobile ? 14 : 15, color: 'var(--text-secondary)', lineHeight: 1.7, margin: 0 }}>
      ...
      {/* FOOTER */}
      <footer style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border-default)', padding: `${isMobile ? 24 : 32}px ${px}px`, display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', justifyContent: 'space-between', gap: isMobile ? 16 : 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)' }}>FixWorkFlow</span>
        </div>
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          <a href="mailto:support@fixworkflow.com" style={{ fontSize: 13, color: 'var(--text-muted)', textDecoration: 'none' }}>Contact</a>
        </div>
        <span style={{ fontSize: 12, color: 'var(--text-faint)' }}>© 2026 FixWorkFlow</span>
      </footer>
    </div>
  );
}
